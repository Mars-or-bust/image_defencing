...
parser.add_argument('--num_layers',
                    default=5,
                    dest='num_layers',
                    help='Number of layers for the network',
                    type=int
                    )
...
model = REDNet10(num_features=args.features, num_layers=args.num_layers)
model= nn.DataParallel(model)

if load:
    checkpoint = torch.load(os.path.join(model_dir, model_name + '/epoch-{0:0>3}.pth'.format(last_epoch)))
    model.load_state_dict(checkpoint)
    loss_list = list(np.load(log_dir + '/' + model_name + '/LossList.npy'))[:last_epoch]
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(loop):
        ...
        # fence_mask = data['fence_mask'].to(device)
    ...
    torch.save(model.state_dict(), os.path.join(model_dir, model_name + '/epoch-{0:0>3}.pth'.format(last_epoch+epoch)))
    
    # Save the loss
    loss_list.append((last_epoch+epoch,loss.item()))
    np.save(log_dir + '/' + model_name + '/LossList.npy', np.array(loss_list))
    ...
    with torch.no_grad():
        for i, data in enumerate(test_dataset):
            ...
            np.save(log_dir + '/' + model_name + '/intermediate_imgs_{0:0>3}'.format(last_epoch+epoch), output.cpu().numpy())
            print('saved!')
